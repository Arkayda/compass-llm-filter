"""Конфигурация прокси из переменных окружения (префикс COMPASS_).

Env задаёт стартовые значения; рабочие настройки (mode/fail_mode) дальше
живут в State и меняются через PUT /v1/settings без перезапуска.
"""
