...
Запоминает каждую подстановку (реальное -> фейковое), поэтому ответ модели
можно «дезанонимизировать» перед показом оператору. Подстановки
детерминированы: seed = SHA256 реального значения, состояние между запросами
не хранится.

Режимы: "fake" (реалистичные подстановки) и "placeholders" ([PHONE]/[EMAIL]/
USER_1, обратная замена только для однозначных плейсхолдеров).
"""
...
# lookbehind обязателен: без него sre пытается применить [\w.+-]+@ с каждой
# позиции длинного слова и откаты дают O(n^2) — 300КБ обычного текста вешали
...
# расширения файлов, которые выглядят как домены
FILE_EXTS = {
...
# латинское имя/ник рядом с этими символами — часть пути/команды/переменной,
# не трогаем
...
# общий ГПСЧ и проверки сумм — в validators
...
def _ru_identifier(digits: str) -> bool:
    """Валидный СНИЛС/ИНН/ОГРН: телефонная фаза пропускает, заменит ru_pii."""
...
class Anonymizer:
    ...
    Порядок фаз в sanitize_string важен: имена -> ники (через маркеры), затем
    ссылки/секреты/почты/упоминания/домены/телефоны/идентификаторы/IP и только
    потом раскрытие маркеров — иначе generic-замены портят уже вставленные
    фейки.
    """
    ...
    def register_entity(self, value: str, kind: str = "auto") -> str | None:
        """Зарегистрировать имя/название компании из приложения; дальше все
        вхождения value в текстах заменяются. Не имя (цифры/мусор) -> None."""
    ...
    def _hits(fake_value: str, avoid) -> bool:
        """Фейк содержит чьё-то реальное имя — надо перегенерить."""
    ...
    def _fake_username(self, name: str, avoid=None) -> str:
        """Ник того же стиля: с/без @, с цифрами, если были."""
    ...
    def prepare(self) -> None:
        ...
        кириллические имена по границам слов, латинские имена/ники — только
        как отдельное слово в прозе (пути и переменные не трогаем)."""
        self.reconcile()
        if not self.name_map:
            self._phases = []
            return

        # от длинных имён к коротким
    ...
    def _fake_domain_repl(self, match: re.Match) -> str:
        """Домен -> фейк; у хостов из 3+ меток сохраняем сервисный префикс
        (chat., mail., api.)."""
        d = match.group(0)
    ...
    def sanitize_string(self, s: str) -> str:
        ...
        if self.fake:
            ...
            def repl_phone(m: re.Match) -> str:
                cand = m.group(0)
                if not _phone_like(cand) or _defer_to_ru_pii(cand):
                    return cand  # дальше разберёт ru_pii
        ...
        # IBAN до телефона и поздним маркером: цифровой хвост IBAN иначе
        # съест телефонная фаза (в т.ч. у уже вставленного фейка)
        s = ru_pii.apply(s, self, only=("ibans",), mark_late=True)
        s = RE_PHONE_CANDIDATE.sub(repl_phone, s)  # счётчик ведёт repl
    ...
    def de_anonymize(self, text: str) -> str:
        ...
        словарные фейки — по границам слов."""
